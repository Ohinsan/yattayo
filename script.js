...
const BASE_TEMPLATES = {
    '1LDK': [
        ...
        { id: 'bath', defaultName: 'お風呂・洗濯', chores: ['浴槽・浴室の掃除', '洗濯', 'タオル交換'] },
    ...
    '2LDK': [
        ...
        { id: 'bath', defaultName: 'お風呂・洗濯', chores: ['浴槽・浴室の掃除', '洗濯', 'タオル交換'] },
        { id: 'toilet', defaultName: 'トイレ', chores: ['トイレ掃除'] },
        { id: 'bedroom', defaultName: '寝室', chores: ['掃除機がけ', '拭き掃除', 'ベッドメイキング'] },
        { id: 'room1', defaultName: '部屋1', chores: ['掃除機がけ', '拭き掃除', 'ベッドメイキング'] },
    ...
    '3LDK+': [
        ...
        { id: 'room1', defaultName: '部屋1', chores: ['掃除機がけ', '拭き掃除', 'ベッドメイキング'] },
        { id: 'washroom', defaultName: '洗面所', chores: ['洗面台の掃除', 'タオル交換'] },
        { id: 'room2', defaultName: '部屋2', chores: ['掃除機がけ', '拭き掃除', 'ベッドメイキング'] },
        { id: 'room3', defaultName: '部屋3', chores: ['掃除機がけ', '拭き掃除', 'ベッドメイキング'] },
        { id: 'trash', defaultName: 'ごみ捨て', chores: ['ごみ捨て'] }
    ]
};
...
function renderQuickAddButtons() {
            ...
            <div class="quick-add-section">
                ...
                <div class="quick-add-buttons">
                    ${section.chores.map(chore => {
                        ...
                        return `<button class="quick-add-btn ${attrClass}" onclick="addQuickChore('${fullName}', this)">${chore}</button>`;
                    }).join('')}
                </div>
            </div>
        `;
    ...
    if (currentFloors >= 2) {
        ...
        if (currentFloors >= 2) {
            stairsHtml += `
                <button class="quick-add-btn attr-wind" onclick="addQuickChore('1F〜2F階段：掃き掃除', this)">1F〜2F階段 掃き</button>
                <button class="quick-add-btn attr-earth" onclick="addQuickChore('1F〜2F階段：拭き掃除', this)">1F〜2F階段 拭き</button>
            `;
        }
        if (currentFloors === 3) {
            stairsHtml += `
                <button class="quick-add-btn attr-wind" onclick="addQuickChore('2F〜3F階段：掃き掃除', this)">2F〜3F階段 掃き</button>
                <button class="quick-add-btn attr-earth" onclick="addQuickChore('2F〜3F階段：拭き掃除', this)">2F〜3F階段 拭き</button>
            `;
        }
...
function saveToLocalStorage() {
    ...
    localStorage.setItem('kajigonUltimate_yattayoState', JSON.stringify(appData));
    localStorage.setItem('kajigonUltimate_status', JSON.stringify(kajigonState));
}

function loadFromLocalStorage() {
    const savedApp = localStorage.getItem('kajigonUltimate_yattayoState');
    if (savedApp) {
...
function getCompanionEmoji() {
    ...
    // Tie-breaker: find the dominant attribute
    const winner = pts.find(p => p.val === maxVal).attr;
...
function getCompanionEmojiString() {
    ...
    if (lv >= 10) {
        // カンスト時、完全バランスなら 🌟、それ以外なら各属性シンボル
        if (kajigonState.firePoints === kajigonState.waterPoints && 
            kajigonState.waterPoints === kajigonState.windPoints && 
            kajigonState.windPoints === kajigonState.earthPoints) {
            return '🐉🌟';
        }
        const winner = pts.find(p => p.val === maxVal);
...
// 👾 相棒表示（絵文字2つだけを静かに表示。カンスト時は「みなおす」「そだてなおす」ボタンを出現！）
...
// ==========================================================================
// --- 🏆 表彰エンディング画面 (完全サプライズ・ドカン！と出現) ---
// ==========================================================================
/*
  ==========================================================================
  🎨 MIDJOURNEY / DALL-E 3 IMAGE GENERATION PROMPTS (アスペクト比 1:1)
  ==========================================================================
  
  ◆ 火 (Fire):
  A powerful legendary fire dragon standing inside a glowing, modern clean kitchen, cinematic lighting, fantasy RPG style, vibrant red and gold colors, high resolution, square composition, background with clean counters and subtle fire sparkles. --ar 1:1
  
  ◆ 水 (Water):
  A beautiful majestic water dragon swimming elegantly inside a sparkling, clean bathroom, Translucent water wings, fantasy RPG style, deep blue and emerald teal colors, high resolution, square composition, background with clean tiled walls. --ar 1:1
  
  ◆ 風 (Wind):
  A swift winged storm dragon floating gracefully inside a bright, sunlit living room, scattering a gentle breeze, fantasy RPG style, pastel purple and silver colors, high resolution, square composition. --ar 1:1
  
  ◆ 土 (Earth):
  A massive ancient earth dragon resting stable inside a cozy, perfectly organized bedroom, fantasy RPG style, deep forest green and wood brown colors, high resolution, square composition. --ar 1:1
...
function openEndingModal() {
    ...
    const textContainer = document.getElementById('ending-scroll-content');
        ...
        // 属性ポイントによるドラゴンの決定
    const pts = [
        { attr: 'fire', val: kajigonState.firePoints, name: '🌋 獄炎龍ボルカノン 🦖', file: 'dragon_fire.png', desc: 'おうちのシンクやコンロをピカピカに磨き上げ、暮らしに熱い情熱の火を灯し続けてくれました。' },
        { attr: 'water', val: kajigonState.waterPoints, name: '🌊 海王神龍リヴァイア 🐙', file: 'dragon_water.png', desc: 'お風呂や洗濯、食器洗いで水回りをピカピカに清め、家族みんなに清潔と潤いをもたらしてくれました。' },
        { attr: 'wind', val: kajigonState.windPoints, name: '🌪️ 嵐神翼龍ゼフィロス 🤖', file: 'dragon_wind.png', desc: '丁寧な掃除機がけや階段の掃き掃除でおうちの埃を吹き飛ばし、常に澄み切った新しい風を送り続けてくれました。' },
        { attr: 'earth', val: kajigonState.earthPoints, name: '🪵 巌帝古龍ガイアード 🌲', file: 'dragon_earth.png', desc: '拭き掃除やゴミ捨て、ベッドメイキングでおうちの基礎をがっしりと支え、最高の安定感を与えてくれました。' }
    ];
    ...
    if (maxVal === 0) {
        selectedDragon = { name: '🌟 創世輝龍アルティマ 🐉', file: 'dragon_all.png', desc: 'すべての家事を愛と調和の心で美しくこなしてくれました。' };
    } else {
        const winner = pts.find(p => p.val === maxVal);
        if (winner) {
            selectedDragon = winner;
        } else {
            selectedDragon = { name: '🌟 創世輝龍アルティマ 🐉', file: 'dragon_all.png', desc: 'すべての家事を愛と調和の心で美しくこなしてくれました。' };
        }
    }
    ...
    if (kajigonState.firePoints === kajigonState.waterPoints && 
        kajigonState.waterPoints === kajigonState.windPoints && 
        kajigonState.windPoints === kajigonState.earthPoints) {
        selectedDragon = { name: '🌟 創世輝龍アルティマ 🐉', file: 'dragon_all.png', desc: 'すべての家事を完璧なバランスでこなし、世界に調和をもたらしました。' };
    }

    // 画像とテキストの設定 (表彰時にサプライズ爆発)
    titleEl.textContent = '🏆 よく頑張りました！';
    imgEl.src = selectedDragon.file;
    imgEl.alt = selectedDragon.name;

    // ローカルファイル未配置時のエラーハンドリング
    ...
    textContainer.innerHTML = `
        ...
        <p style="font-size: 16px; font-weight: 800; color: #b45309; text-align: center; margin: 0.5rem 0;">【 ${selectedDragon.name} 】</p>
        <p>へと最終変化を遂げました。</p>
        <p style="font-size: 12px; color: var(--text-sub); border-left: 3px solid var(--primary); padding-left: 0.5rem; margin: 0.75rem 0;">
            "${selectedDragon.desc}"
        </p>
...
function getLuckyChoreForDate(dateStr) {
    ...
    const index = Math.abs(hash) % chores.length;
    return chores[index];
}
...
function updateLuckyChore() {
    ...
    if (badgeContainer) {
        ...
        badgeContainer.innerHTML = `
            <div class="lucky-chore-badge" style="${isDoneToday ? 'background: #e2e8f0; border-color: #cbd5e1; color: #94a3b8; text-decoration: line-through;' : ''}">
